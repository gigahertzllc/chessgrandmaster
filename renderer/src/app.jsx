 ...
 *   - NEW: Editorial theme (magazine style, gold accent)
 *   - Each theme has dark and light variants
 *   - Admin Panel: Photo selection from Wikipedia (multiple image options)
 *   - Uses Wikipedia Action API (Method 3) for reliable image loading
 *   - Zone Mode updated with theme color schemes
 *
 * v3.3.0 - Major Fixes
 *   - FIXED: Classic Dark is now the DEFAULT theme
 *   - FIXED: Masters is now the DEFAULT tab
 *   - FIXED: All emojis removed, using text symbols only
 *   - FIXED: Wikipedia images use <img> tags with referrerPolicy
 *   - ADDED: "Seed to DB" in Admin Panel to save built-in players to Supabase
 *   - ADDED: db.seedBuiltInPlayers() and db.getAllPlayers() functions
 *
...
const THEMES = {
  ...
  // NEW: Gallery Theme (v2-design-5 - Experimental/Museum)
  "gallery": {
    id: "gallery", name: "Gallery",
    bg: "#0C0C0C", bgAlt: "#141414",
    card: "#141414", cardHover: "#1A1A1A",
    ink: "#FFFFFF", inkMuted: "#666666", inkFaint: "#444444",
    accent: "#FF5C00", accentSoft: "rgba(255, 92, 0, 0.15)",
    success: "#4CAF50", error: "#EF5350",
    border: "rgba(255, 255, 255, 0.08)", borderStrong: "rgba(255, 255, 255, 0.15)",
    shadow: "0 4px 24px rgba(0, 0, 0, 0.5)", shadowStrong: "0 12px 48px rgba(0, 0, 0, 0.7)",
    themeClass: "gallery"
  ...
  "gallery-light": {
    ...
    bg: "#F5F5F5", bgAlt: "#EBEBEB",
    card: "#FFFFFF", cardHover: "#F5F5F5",
    ink: "#0C0C0C", inkMuted: "#666666", inkFaint: "#999999",
    accent: "#FF5C00", accentSoft: "rgba(255, 92, 0, 0.1)",
    success: "#2E7D32", error: "#C62828",
    border: "rgba(0, 0, 0, 0.08)", borderStrong: "rgba(0, 0, 0, 0.15)",
    shadow: "0 2px 8px rgba(0, 0, 0, 0.08)", shadowStrong: "0 8px 32px rgba(0, 0, 0, 0.12)",
    themeClass: "gallery"
  },
  // NEW: Typographic Theme (v2-design-2 - Pentagram/Paula Scher)
  "typographic": {
    id: "typographic", name: "Typographic",
    bg: "#1A1A1A", bgAlt: "#242424",
    card: "#242424", cardHover: "#2E2E2E",
    ink: "#F0EDE6", inkMuted: "#999999", inkFaint: "#666666",
    accent: "#FF4D00", accentSoft: "rgba(255, 77, 0, 0.15)",
    success: "#4CAF50", error: "#EF5350",
    border: "rgba(255, 255, 255, 0.1)", borderStrong: "rgba(255, 255, 255, 0.2)",
    shadow: "0 4px 24px rgba(0, 0, 0, 0.5)", shadowStrong: "0 12px 48px rgba(0, 0, 0, 0.7)",
    themeClass: "typographic"
  },
  "typographic-light": {
    id: "typographic-light", name: "Typographic Light",
    bg: "#F0EDE6", bgAlt: "#E5E2DB",
    card: "#FFFFFF", cardHover: "#F0EDE6",
    ink: "#1A1A1A", inkMuted: "#666666", inkFaint: "#999999",
    accent: "#FF4D00", accentSoft: "rgba(255, 77, 0, 0.1)",
    success: "#2E7D32", error: "#C62828",
    border: "rgba(0, 0, 0, 0.1)", borderStrong: "rgba(0, 0, 0, 0.2)",
    shadow: "0 2px 8px rgba(0, 0, 0, 0.08)", shadowStrong: "0 8px 32px rgba(0, 0, 0, 0.12)",
    themeClass: "typographic"
  },
  // NEW: Editorial Theme (design-1-editorial - Magazine/Newspaper)
  "editorial": {
    id: "editorial", name: "Editorial",
    bg: "#0A0A0B", bgAlt: "#111112",
    card: "#111112", cardHover: "#181819",
    ink: "#F5F5F3", inkMuted: "#888886", inkFaint: "#555553",
    accent: "#C9A227", accentSoft: "rgba(201, 162, 39, 0.15)",
    success: "#3D8B40", error: "#C23B22",
    border: "rgba(255, 255, 255, 0.08)", borderStrong: "rgba(255, 255, 255, 0.15)",
    shadow: "0 4px 24px rgba(0, 0, 0, 0.5)", shadowStrong: "0 12px 48px rgba(0, 0, 0, 0.7)",
    themeClass: "editorial"
  },
  "editorial-light": {
    id: "editorial-light", name: "Editorial Light",
    bg: "#FAFAF8", bgAlt: "#F5F5F3",
    card: "#FFFFFF", cardHover: "#F5F5F3",
    ink: "#1A1A1A", inkMuted: "#666664", inkFaint: "#999997",
    accent: "#8B6914", accentSoft: "rgba(139, 105, 20, 0.1)",
    success: "#2E7D32", error: "#C62828",
    border: "rgba(0, 0, 0, 0.08)", borderStrong: "rgba(0, 0, 0, 0.15)",
    shadow: "0 2px 8px rgba(0, 0, 0, 0.06)", shadowStrong: "0 8px 32px rgba(0, 0, 0, 0.12)",
    themeClass: "editorial"
...
export default function App() {
  ...
  // Theme state
  const [themeId, setThemeId] = useState(() => localStorage.getItem("cm-theme") || "gallery");
  ...
  const [playerOverrides, setPlayerOverrides] = useState({}); // Custom images/data from Supabase
  ...
  useEffect(() => {
    ...
    document.body.classList.remove('theme-gallery', 'theme-typographic', 'theme-editorial', 'theme-classic', 'theme-modern');
    document.body.classList.remove('light', 'dark');
    
    // Add new theme class based on themeId
    if (theme.themeClass) {
      document.body.classList.add(`theme-${theme.themeClass}`);
    } else if (theme.isClassic) {
      document.body.classList.add('theme-classic');
    } else {
      document.body.classList.add('theme-modern');
    }
    
    // Add light/dark modifier
    const isLight = themeId.includes('light') || themeId === 'sepia';
  ...
  // Load player overrides (custom images, etc.) from Supabase
  const loadPlayerOverrides = async () => {
    try {
      const { data, error } = await db.getAllPlayerOverrides();
      if (error) throw error;
      if (data) {
        const overridesMap = {};
        data.forEach(o => {
        ...
        console.log("Loaded player overrides:", Object.keys(overridesMap).length);
      }
    } catch (e) {
      ...
      {isMobile && !showZoneMode && !showChessCoach && !showAdminPanel && (
        <nav className="mobile-nav" style={{
          ...
          {[
            ...
            { id: "play", label: "Play", icon: "▶", classicIcon: "▶" },
        ...
        <div style={{
          ...
          <div style={{
            ...
            <AdminPanel 
              ...
              onPlayersUpdated={() => { loadCustomPlayers(); loadPlayerOverrides(); }}
            />
          </div>
        </div>
      )}
